...
user_problem_statement: "🧩 Minimal Fix Request for Emergent AI - Please apply only the following targeted production issues: 1) Remove Persistent Video Overlay - Hide it on play or allow user to dismiss via close button. 2) Enable Video Player Controls - Turn on full controls (use native or configure custom player as needed). 3) Fix Progress Graph - The progress bar always shows 0 minutes watched, ensure watch_progress is saved and rendered per user. 4) Fix Admin Auth for Filters - Admins get 'Authentication required' when editing Topics, Guides, or Countries. 5) Update Social Links - Replace existing social URLs with: Instagram: https://instagram.com/fiestaenglish, TikTok: https://www.tiktok.com/@english_fiesta, YouTube: https://www.youtube.com/@englishfiesta. 6) Enable CMS Editing - CMS fields (FAQ, About, Buttons) don't update, ensure changes save to backend and persist in database."
